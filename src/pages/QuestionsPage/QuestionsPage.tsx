...
import { ArrowRight, Upload, XCircle, AlertTriangle, HelpCircle } from 'lucide-react';
...
function QuestionsPage() {
  ...
  const { questions, loading, error, clearError, addItem, addMany, updateItem, deleteItem } =
  ...
  // چک محدودیت پلن فقط برای نقش Instructor؛ SuperAdmin محدودیتی نداره
  ...
  return (
    <AppLayout title={exam ? `بانک سوال — ${exam.title}` : 'بانک سوال'}>
      {error && <Toast message={error} tone="danger" icon={XCircle} onDismiss={clearError} />}
      ...
      {loading ? (
        ...
        <>
          <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
            <h1 className="text-2xl font-bold dark:text-white">سوالات {exam?.title}</h1>
